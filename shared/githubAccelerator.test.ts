...
test("prefixes GitHub URLs and keeps a direct fallback candidate", () => {
  const raw = "https://github.com/DeraDream/Forwardx/releases/download/v1.2.3/panel.tar.gz";
  assert.equal(
...
test("builds an accelerated installer command with a persistent script argument", () => {
  assert.equal(
    ...
    "curl -fsSL 'https://mirror.example.com/https://raw.githubusercontent.com/DeraDream/Forwardx/main/scripts/install-panel-docker.sh' | sudo bash -s -- upgrade --github-accelerator 'https://mirror.example.com'",
  );
});
