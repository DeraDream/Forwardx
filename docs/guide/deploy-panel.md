...
FORWARDX_IMAGE=ghcr.io/deradream/forwardx:latest
...
services:
  forwardx:
    image: ${FORWARDX_IMAGE:-ghcr.io/deradream/forwardx:latest}
...
docker image ls --no-trunc --format '{{.Repository}} {{.Tag}} {{.ID}}' ghcr.io/deradream/forwardx \
  | awk -v current="$CURRENT_IMAGE_ID" '$1 == "ghcr.io/deradream/forwardx" && $2 != "<none>" && $3 != current { print $1 ":" $2 }' \
...
```bash
curl -fsSL https://raw.githubusercontent.com/DeraDream/Forwardx/main/scripts/install-panel-local.sh \
...
mkdir -p "$APP_DIR"
curl -fL "https://github.com/DeraDream/Forwardx/releases/download/${VERSION}/forwardx-panel-${VERSION}.tar.gz" -o /tmp/forwardx-panel.tar.gz
