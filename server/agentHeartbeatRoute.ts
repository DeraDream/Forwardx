...
function landingServiceAction(service: any) {
  ...
  const installRuntime = `missing=0; for bin in curl tar xz sha256sum base64 find install awk grep ss mktemp mkdir head uname; do command -v "$bin" >/dev/null 2>&1 || missing=1; done; if [ "$missing" = 1 ]; then if command -v apt-get >/dev/null 2>&1; then apt-get update -qq && apt-get install -y -qq ca-certificates curl xz-utils tar coreutils findutils gawk grep iproute2; elif command -v dnf >/dev/null 2>&1; then dnf install -y -q ca-certificates curl xz tar coreutils findutils gawk grep iproute; elif command -v yum >/dev/null 2>&1; then yum install -y -q ca-certificates curl xz tar coreutils findutils gawk grep iproute; elif command -v zypper >/dev/null 2>&1; then zypper -n install ca-certificates curl xz tar coreutils findutils gawk grep iproute2; elif command -v apk >/dev/null 2>&1; then apk add --no-cache ca-certificates curl xz tar coreutils findutils gawk grep iproute2; elif command -v pacman >/dev/null 2>&1; then pacman -Sy --noconfirm ca-certificates curl xz tar coreutils findutils gawk grep iproute2; else echo "[landing] missing runtime dependencies and no supported package manager" >&2; exit 1; fi; fi; for bin in curl tar xz sha256sum base64 find install awk grep ss mktemp mkdir head uname; do command -v "$bin" >/dev/null 2>&1 || { echo "[landing] missing dependency: $bin" >&2; exit 1; }; done; if [ ! -x ${shQuote(`${LANDING_SS_RUNTIME_DIR}/ssserver`)} ]; then arch=$(uname -m); case "$arch" in x86_64|amd64) asset="shadowsocks-v${version}.x86_64-unknown-linux-musl.tar.xz";; aarch64|arm64) asset="shadowsocks-v${version}.aarch64-unknown-linux-musl.tar.xz";; *) echo "[landing] unsupported CPU architecture: $arch"; exit 1;; esac; tmp=$(mktemp -d); trap 'rm -rf "$tmp"' EXIT; base="https://github.com/shadowsocks/shadowsocks-rust/releases/download/v${version}"; curl -fL --retry 3 --connect-timeout 15 "$base/$asset" -o "$tmp/runtime.tar.xz"; curl -fL --retry 3 --connect-timeout 15 "$base/$asset.sha256" -o "$tmp/runtime.tar.xz.sha256"; expected=$(awk '{print $1}' "$tmp/runtime.tar.xz.sha256"); test -n "$expected"; printf '%s  %s\\n' "$expected" "$tmp/runtime.tar.xz" | sha256sum -c -; mkdir -p ${shQuote(LANDING_SS_RUNTIME_DIR)}; tar -xJf "$tmp/runtime.tar.xz" -C "$tmp"; bin=$(find "$tmp" -type f -name ssserver -perm -u+x | head -n 1); test -n "$bin"; install -m 0755 "$bin" ${shQuote(`${LANDING_SS_RUNTIME_DIR}/ssserver`)}; fi`;
  return {
    ...
    commands: [
      ...
      addInputRules,
      `for bin in iptables ip6tables; do command -v $bin >/dev/null 2>&1 || continue; $bin -t mangle -C PREROUTING -p tcp --dport ${port} -m conntrack --ctstate NEW -m comment --comment fwx-landing-${id}:conn -j CONNMARK --restore-mark 2>/dev/null || $bin -t mangle -I PREROUTING -p tcp --dport ${port} -m conntrack --ctstate NEW -m comment --comment fwx-landing-${id}:conn -j CONNMARK --restore-mark; for proto in tcp udp; do $bin -t mangle -C PREROUTING -p $proto --dport ${port} -m comment --comment fwx-landing-${id}:in -j CONNMARK --restore-mark 2>/dev/null || $bin -t mangle -I PREROUTING -p $proto --dport ${port} -m comment --comment fwx-landing-${id}:in -j CONNMARK --restore-mark; $bin -t mangle -D POSTROUTING -p $proto --sport ${port} -m comment --comment fwx-landing-${id}:out -j CONNMARK --restore-mark 2>/dev/null || true; $bin -t mangle -C POSTROUTING -p $proto -m conntrack --ctorigdstport ${port} -m comment --comment fwx-landing-${id}:out -j CONNMARK --restore-mark 2>/dev/null || $bin -t mangle -I POSTROUTING -p $proto -m conntrack --ctorigdstport ${port} -m comment --comment fwx-landing-${id}:out -j CONNMARK --restore-mark; done; done`,
...
agentRouter.post("/api/agent/heartbeat", async (req: Request, res: Response) => {
  ...
  try {
    ...
    // The landing-host switch is a host-level circuit breaker.  Keep the
    // services in the database (and keep rules that refer to them intact),
    // but reconcile every runtime unit as removed while the host is disabled.
    const landingHost = await db.getLandingHostByHostId(Number(host.id)) as any;
    const landingHostEnabled = !landingHost || landingHost.isEnabled !== false;
    for (const service of await db.getLandingServicesForHost(Number(host.id), true, true)) {
      const action = landingServiceAction(landingHostEnabled ? service : { ...service, isEnabled: false });
    ...
    for (const task of await getFullChainRuntimeTasks(Number(host.id))) {
      ...
      if (task.portStatus === "checking") actions.push({
        ...
        commands: [Number(task.replacesChainId) > 0
          ? "# replacement chain: the old chain is intentionally still listening on this port; DB validation already excludes its rules"
          : `port_hex=$(printf '%04X' ${port}); if { command -v ss >/dev/null 2>&1 && ss -ltnu | awk '{print $5}' | grep -Eq "[:.]${port}$"; } || grep -qi ":$port_hex " /proc/net/tcp /proc/net/tcp6 /proc/net/udp /proc/net/udp6 2>/dev/null; then echo "端口不可用: ${port}" >&2; exit 1; fi`],
      });
    ...
    if (landingHostEnabled) {
      for (const service of await db.getLandingServicesForHost(Number(host.id), false)) {
        const targetIp = String((service as any).latencyTargetHost || "").trim();
        const targetPort = Number((service as any).latencyTargetPort || 443);
        if (!targetIp || !Number.isInteger(targetPort) || targetPort < 1 || targetPort > 65535) continue;
        hostProbeServices.push({
          serviceId: Number((service as any).id),
          landingServiceId: Number((service as any).id),
          targetIp,
          targetPort,
          method: "tcping",
          intervalSeconds: 60,
