...
import { Activity, ArrowDownToLine, ArrowRightLeft, ArrowUpFromLine, ChartNoAxesCombined, Copy, GitBranch, Link2, Pencil, QrCode, Server, Trash2 } from "lucide-react";
...
export function LandingManagement({ viewMode = "card", createRequestKey }: { viewMode?: LandingViewMode; createRequestKey?: number }) {
  ...
  const openCreate = () => {
    ...
    setEditing(null); setName("落地 SS"); setLatencyTargetHost(DEFAULT_LANDING_LATENCY_TARGET); setLatencyTargetPort(443); setPortCheckText(""); setOpen(true);
  };
  ...
  const card = (service: any) => <Card key={service.id} className="w-full border-border/40 bg-card/60 backdrop-blur-md"><CardContent className={viewMode === "compact" ? "space-y-2.5 p-3" : "space-y-3 p-4"}><div className="flex items-start justify-between gap-3"><div className="min-w-0"><div className="truncate font-medium">{service.name}</div><p className="mt-1 truncate text-xs text-muted-foreground">用户：当前用户 · {service.host?.name || `主机 #${service.hostId}`}</p></div><div className="flex shrink-0 items-center gap-2"><Switch checked={service.isEnabled !== false} disabled={toggle.isPending || service.status === "removing"} onCheckedChange={isEnabled => toggle.mutate({ id: service.id, isEnabled })} aria-label={`${service.name} 启用状态`} /></div></div><div className={viewMode === "compact" ? "space-y-1.5 text-xs" : "space-y-2 text-sm"}><div className={viewMode === "compact" ? "flex min-w-0 items-center justify-between gap-3" : "flex min-w-0 items-center justify-between gap-3 rounded-md border p-2"}><span className="shrink-0 text-muted-foreground">入口</span><span className="min-w-0 break-all text-right font-mono">{service.host?.ip || "-"}:{service.port}</span></div><div className={viewMode === "compact" ? "flex min-w-0 items-center justify-between gap-3" : "flex min-w-0 items-center justify-between gap-3 rounded-md border p-2"}><span className="shrink-0 text-muted-foreground">出口（公网）</span><span className="min-w-0 break-all text-right font-mono">{endpointOf(service) || "-"}:{service.port}</span></div></div><div className="flex flex-wrap items-center gap-1.5 text-xs"><Badge variant="outline" className="h-5 px-1.5 text-[10px]">{String(service.protocol).toUpperCase()}</Badge><Badge variant="secondary" className="h-5 px-1.5 text-[10px]">TCP + UDP</Badge><Badge variant={statusVariant(service)} className="h-5 px-1.5 text-[10px]">{serviceStatus(service)}</Badge></div><div className="grid grid-cols-2 gap-x-3 gap-y-2 border-t border-border/40 pt-2 text-xs"><div className="min-w-0"><div className="mb-1 text-muted-foreground">24H 入向</div><div className="flex items-center gap-1 text-emerald-600"><ArrowDownToLine className="h-3 w-3" />{formatBytes(service.traffic?.bytesIn24h)}</div></div><div className="min-w-0"><div className="mb-1 text-muted-foreground">24H 出向</div><div className="flex items-center gap-1 text-amber-600"><ArrowUpFromLine className="h-3 w-3" />{formatBytes(service.traffic?.bytesOut24h)}</div></div><div className="min-w-0"><div className="mb-1 text-muted-foreground">累计流量</div><div className="flex items-center gap-1"><ArrowRightLeft className="h-3 w-3" />{formatBytes(Number(service.traffic?.bytesInTotal || 0) + Number(service.traffic?.bytesOutTotal || 0))}</div></div><div className="min-w-0"><div className="mb-1 text-muted-foreground">延迟</div><div className={"flex items-center gap-1 " + (service.latestLatencyIsTimeout ? "text-destructive" : "text-emerald-600")}><Activity className="h-3 w-3" />{service.latestLatencyIsTimeout ? "超时" : service.latestLatencyMs ? `${service.latestLatencyMs}ms` : "等待探测"}</div></div></div><div className="flex justify-end border-t border-border/40 pt-2"><Button size="icon" variant="ghost" title="编辑" onClick={() => openEdit(service)}><Pencil className="h-4 w-4" /></Button>{actionButtons(service)}</div></CardContent></Card>;
