...
function LandingProbeDialog({ service, open, onOpenChange }: { service: any | null; open: boolean; onOpenChange: (value: boolean) => void }) {
  ...
  const services = trpc.landing.list.useQuery(undefined, { enabled: open && serviceId > 0, refetchInterval: open ? 900 : false });
  ...
  useEffect(() => {
    if (!open) { started.current = false; setStartedAt(0); return; }
    if (serviceId > 0 && !started.current) { started.current = true; start.mutate({ id: serviceId }); }
  }, [open, serviceId]);
  const target = displayLatencyTarget(service);
  return <Dialog open={open} onOpenChange={onOpenChange}><DialogContent className="max-w-lg"><DialogTitle>延迟探测</DialogTitle><DialogDescription>{service?.name || "落地 SS"} · 目标：{target}</DialogDescription><div className="py-8 text-center"><div className={completed ? (current?.latestLatencyIsTimeout ? "text-xl font-semibold text-destructive" : "text-2xl font-semibold text-emerald-600") : "text-lg font-medium"}>{!startedAt || start.isPending ? "正在下发探测…" : completed ? (current?.latestLatencyIsTimeout ? "探测超时" : `${current?.latestLatencyMs} ms`) : "正在连续 TCPing 3 次…"}</div><p className="mt-3 text-sm text-muted-foreground">Agent 将完成 3 次 TCP 连接采样、汇总后停止，并把结果写入面板延迟记录。</p></div><DialogFooter><Button onClick={() => onOpenChange(false)}>{completed ? "完成" : "后台继续"}</Button></DialogFooter></DialogContent></Dialog>;
}
...
export function LandingManagement({ viewMode = "card", createRequestKey }: { viewMode?: LandingViewMode; createRequestKey?: number }) {
  ...
  const card = (service: any) => <Card key={service.id} className="w-full max-w-[34rem] border-border/45 bg-card/65 backdrop-blur-md"><CardContent className={viewMode === "compact" ? "p-3" : "p-4"}><div className="flex items-start justify-between gap-3"><div className="min-w-0"><div className="truncate font-medium">{service.name}</div><p className="mt-1 truncate text-sm text-muted-foreground">用户：当前用户 · {service.host?.name || `主机 #${service.hostId}`}</p></div><div className="flex shrink-0 items-center gap-1"><Button size="icon" variant="ghost" className="h-7 w-7" title="编辑" onClick={() => openEdit(service)}><Pencil className="h-3.5 w-3.5" /></Button><Badge variant={statusVariant(service)}>{serviceStatus(service)}</Badge></div></div><div className={viewMode === "compact" ? "mt-3 space-y-1.5 text-xs" : "mt-4 space-y-2 text-sm"}><div className={viewMode === "compact" ? "flex min-w-0 items-center justify-between gap-3" : "flex min-w-0 items-center justify-between gap-3 rounded-md border p-2"}><span className="shrink-0 text-muted-foreground">入口</span><span className="min-w-0 break-all text-right font-mono">{service.host?.ip || "-"}:{service.port}</span></div><div className={viewMode === "compact" ? "flex min-w-0 items-center justify-between gap-3" : "flex min-w-0 items-center justify-between gap-3 rounded-md border p-2"}><span className="shrink-0 text-muted-foreground">出口（公网）</span><span className="min-w-0 break-all text-right font-mono">{endpointOf(service) || "-"}:{service.port}</span></div></div><div className="mt-3 flex flex-wrap items-center gap-2 border-t pt-3 text-sm"><span className="flex flex-wrap items-center gap-1">落地协议 <Badge variant="outline">{String(service.protocol).toUpperCase()}</Badge><Badge variant="outline">TCP + UDP</Badge></span></div><div className="mt-3 grid grid-cols-2 gap-3 border-t pt-3 text-sm sm:grid-cols-4"><div><div className="text-xs text-muted-foreground">24H 入向</div><div className="text-emerald-600">↓ {formatBytes(service.traffic?.bytesIn24h)}</div></div><div><div className="text-xs text-muted-foreground">24H 出向</div><div className="text-amber-600">↑ {formatBytes(service.traffic?.bytesOut24h)}</div></div><div><div className="text-xs text-muted-foreground">累计流量</div><div>↔ {formatBytes(Number(service.traffic?.bytesInTotal || 0) + Number(service.traffic?.bytesOutTotal || 0))}</div></div><div><div className="text-xs text-muted-foreground">延迟</div><div className={service.latestLatencyIsTimeout ? "text-destructive" : "text-emerald-600"}>⌁ {service.latestLatencyIsTimeout ? "超时" : service.latestLatencyMs ? `${service.latestLatencyMs}ms` : "等待探测"}</div></div></div><div className="mt-4 border-t pt-3">{actionButtons(service)}</div></CardContent></Card>;
