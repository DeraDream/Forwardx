...
[![Latest Release](https://img.shields.io/github/v/release/DeraDream/Forwardx?display_name=tag&sort=semver)](https://github.com/DeraDream/Forwardx/releases/latest)
[![Node.js](https://img.shields.io/badge/Node.js-22+-339933?logo=node.js&logoColor=white)](https://nodejs.org/)
[![Docker](https://img.shields.io/badge/Docker-Ready-2496ED?logo=docker&logoColor=white)](https://www.docker.com/)

## 链接

- [使用文档](https://deradream.github.io/Forwardx/)
- [GitHub Releases](https://github.com/DeraDream/Forwardx/releases/latest)
- [Telegram 群组](https://t.me/ForwardX_panel)
- [Android APK](https://github.com/DeraDream/Forwardx/releases/latest)
...
```bash
curl -fsSL https://raw.githubusercontent.com/DeraDream/Forwardx/main/scripts/install-panel-docker.sh | bash -s -- install
```

升级：

```bash
curl -fsSL https://raw.githubusercontent.com/DeraDream/Forwardx/main/scripts/install-panel-docker.sh | bash -s -- upgrade
```

卸载：

```bash
curl -fsSL https://raw.githubusercontent.com/DeraDream/Forwardx/main/scripts/install-panel-docker.sh | bash -s -- uninstall
```

Docker 默认拉取 `ghcr.io/deradream/forwardx:latest`，数据库配置和 SQLite 数据保存在数据卷中。升级会保留 `.env`、数据卷和部署目录中的 `data/`；卸载脚本仅在用户确认后删除这些数据。
...
```bash
curl -fsSL https://raw.githubusercontent.com/DeraDream/Forwardx/main/scripts/install-panel-local.sh | bash -s -- install
```

升级：

```bash
curl -fsSL https://raw.githubusercontent.com/DeraDream/Forwardx/main/scripts/install-panel-local.sh | bash -s -- upgrade
```

卸载：

```bash
curl -fsSL https://raw.githubusercontent.com/DeraDream/Forwardx/main/scripts/install-panel-local.sh | bash -s -- uninstall
```
...
```bash
# Docker 安装
curl -fsSL "https://mirror.example.com/https://raw.githubusercontent.com/DeraDream/Forwardx/main/scripts/install-panel-docker.sh" \
  | bash -s -- install --github-accelerator "https://mirror.example.com"

# 本地 systemd 安装
curl -fsSL "https://mirror.example.com/https://raw.githubusercontent.com/DeraDream/Forwardx/main/scripts/install-panel-local.sh" \
  | bash -s -- install --github-accelerator "https://mirror.example.com"
```
...
该参数不代理 `ghcr.io` 镜像拉取。Docker 镜像源需要通过 `FORWARDX_IMAGE` 或 `FORWARDX_IMAGE_REPO` 单独配置。详细用法见[部署面板](https://deradream.github.io/Forwardx/guide/deploy-panel)和[升级与备份](https://deradream.github.io/Forwardx/guide/upgrade-backup)。
...
```bash
curl -fsSL https://raw.githubusercontent.com/DeraDream/Forwardx/main/scripts/install-mimic.sh | sudo bash
```
...
数据库地址、反向代理和升级相关变量见[环境变量文档](https://deradream.github.io/Forwardx/guide/env-vars)。MySQL/PostgreSQL 连接池由面板根据主机数量自动管理。常用变量如下：
...
| `FORWARDX_IMAGE` | `ghcr.io/deradream/forwardx:latest` | Docker 镜像 |
...
[![Stargazers over time](https://starchart.cc/DeraDream/Forwardx.svg)](https://starchart.cc/DeraDream/Forwardx)
